...
class MinesweeperAI():
    ...
    def check_knowledge(self):
        ...
        for sentence in knowledge_copy:
            if len(sentence.known_safes()) > 0:
                #adding to a set that is being modified is risky
                for s in sentence.known_safes().copy():
                    self.mark_safe(s)
                    self.check_knowledge()
            if len(sentence.known_mines()) > 0:
                #adding to a set that is being modified is risky
                for s in sentence.known_mines().copy():
                    self.mark_mine(s)
                    self.check_knowledge()
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(-1,2):
            for j in range(-1,2):
                ...
                if (new_row, new_col) in self.mines:
                    count -= 1
                elif (new_row, new_col) in self.safes:
                    continue
                else:
        ...
        # generate code that constantly updates the knowledge base using the functions check_knowledge and create_new_sentences
        while True:
            knowledge_copy = copy.deepcopy(self.knowledge)
            # Check knowledge and update the knowledge base
            self.check_knowledge()
            self.create_new_sentences()
            # Break the loop if no new knowledge is added
            if len(self.knowledge) == len(knowledge_copy):
                break
    ...
    def make_safe_move(self):
        ...
        """
        for valid in self.safes - self.moves_made:
            return valid
